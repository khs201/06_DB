...
----------------------------------------------------------------------------------
-- 6. 스칼라 서브쿼리 (SELECT절 작성하는 단일행 서브쿼리 *꼭 서브쿼리는 아니어도 된다)

--    SELECT절에 사용되는 서브쿼리 결과로 1행만 반환
--    SQL에서 단일 값을 가르켜 '스칼라'라고 함
-- 각 직원들이 속한 직급의 급여 평균 조회
SELECT EMP_NAME, JOB_CODE, JOB_CODE || '의 평균 급여',
(SELECT FLOOR(AVG(SALARY))FROM EMPLOYEE SUB WHERE SUB.JOB_CODE = MAIN.JOB_CODE)  "평균 급여"
FROM EMPLOYEE MAIN; 
...
NVL(MANAGER_ID, '없음'),
NVL(
(SELECT EMP_NAME FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID), '없음') "관리자명"
FROM EMPLOYEE MAIN;
...
SELECT EMP_ID , EMP_NAME , DEPT_CODE , JOB_CODE , HIRE_DATE 
FROM EMPLOYEE e 
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE , JOB_CODE  FROM EMPLOYEE e2  WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000);

-- 5. 77년생 여자 사원과 동일한 부서이면서 동일한 사수를 가지고 있는 사원을 조회하시오 
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 고용일 
SELECT EMP_ID , EMP_NAME, DEPT_CODE , MANAGER_ID , EMP_NO , HIRE_DATE 
FROM EMPLOYEE e 
WHERE (DEPT_CODE, e.MANAGER_ID) = (SELECT DEPT_CODE, MANAGER_ID 
FROM EMPLOYEE e 
...
SELECT EMP_NO , EMP_NAME, JOB_NAME , FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'YYMMDD') ) / 12) 나이, 
CASE 
	WHEN BONUS IS NOT NULL
	THEN (SALARY * BONUS) + SALARY 
	WHEN BONUS IS NULL
	THEN SALARY 
END "보너스 포함 연봉"
FROM EMPLOYEE e
JOIN JOB USING (JOB_CODE)
WHERE TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'YYMMDD') IN (
    SELECT MAX(TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'YYMMDD')) 주번
    FROM EMPLOYEE
    GROUP BY JOB_CODE 
)
ORDER BY 나이 DESC
