...
--> 반환값 중 정수 부분은 차이나는 개월 수
SELECT
	MONTHS_BETWEEN('2024-03-28', SYSDATE)
FROM
	DUAL;
...
-- 다음달 첫 번째, 마지막 날짜
SELECT LAST_DAY(SYSDATE) + 1, -- 첫 번째 날
LAST_DAY(ADD_MONTHS(SYSDATE, 1)) -- 마지막 날
FROM DUAL;
...
SELECT
	1234,
	TO_CHAR(1234) -- 문자열 1234 반환 // 타입이 바뀜
FROM
	DUAL;
	
SELECT
	1234,
	TO_CHAR(1234, '99999999'), -- 앞을 공백으로 채워서 자릿수를 채움
	TO_CHAR(1234, '00000000') -- 앞을 공백으로 채워서 자릿수를 채움
FROM
	DUAL;
...
-- 년,월,일 같은 한글 또는 날짜와 관련없는 문자는 패턴X
...
SELECT
	TO_DATE('2월 27일 화요일 12시 24분', 'MM"월" DD"일" DAY HH"시" MI"분"')
FROM
	DUAL;
...
-- 50 이상
SELECT TO_DATE('50-11-25', 'YY-MM-DD') "YY", --2049
			 TO_DATE('50-11-25', 'RR-MM-DD') "RR"  --2049
FROM DUAL;
...
-- 이름, 급여, 성과급(급여 * 보너스)
-- 단 성과급이 없으면 0으로 표시
-- NULL이랑 산술연산을 수행 시 결과는 무조건 NULL이다!!!!!!!!!!!!!!!!!!
...
SELECT
	...
	JOB_CODE ,
	SALARY ,
	DECODE(SUBSTR(JOB_CODE, 2, 1), '7', SALARY * 1.1 , '6', SALARY * 1.15, '5', SALARY * 1.2, SALARY * 1.05) 보너스
FROM
	EMPLOYEE e;
...
ORDER BY
JOB_CODE;
-- SELECT 절에 작성된 칼럼이 아니라고해서 ORDER BY절에서 사용 못하는게 아님!!!
--> FROM 절 EMPLOYEE 안에 포함되어있기 때문에!!!
-------------
/************* 그룹 함수 *************/
...
/*1*/FROM EMPLOYEE e -- 1) EMPLOYEE 테이블 23행 중
...
/*3*/ SELECT SUM(SALARY) 
/*1*/FROM EMPLOYEE e 
/*2*/WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000;
...
-- 전화번호가 있는 사원의 수 V2
--> NULL이 아닌 행의 수만 카운트
SELECT COUNT(PHONE) FROM EMPLOYEE e ; -- 20행
