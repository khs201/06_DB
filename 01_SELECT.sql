...
SELECT
	SYSDATE - TO_DATE('2024-02-25', 'YYYY-MM-DD')
FROM
	DUAL;
-- EMPLOYEE 테이블에서 
...
-- 현재 시간 - 생년월일
SELECT
	CEIL( (SYSDATE - TO_DATE('1990-01-16', 'YYYY-MM-DD')) / 365 )
FROM
	DUAL;
----------------------------------------------------------------
...
FROM
	EMPLOYEE
WHERE
	SALARY NOT BETWEEN 3000000 AND 5000000;
...
ORDER BY 2 DESC; -- 권장되지 않음, 수정이 불편하기 때문

/* ORDER BY 절에 수식 적용 */
-- EMPLOYEE 테이블에서 이름, 연봉을 연봉 내림차순으로 조회
SELECT  EMP_NAME , SALARY * 12
FROM EMPLOYEE e 
ORDER BY SALARY * 12 DESC; -- 정렬 시 SELECT절에 작성된 컬럼을 그대로 따라 적는 경우가 많다(대부분)!

/* ORDER BY 절에서 별칭 사용하기 */
--> SELECT절 해석 이후 ORDER BY절이 해석되기 때문에, SELECT절에서 해석된 별칭을 ORDER BY 절에서 사용할 수 있다
...
--FROM
--	EMPLOYEE e
-- WHERE 부서코드 = 'D6' -- 에러 발생 "부서코드" 컬럼이 존재하지 않음
-- WHERE 절에는 별칭 사용 불가 해석순서가 더 빠르기 때문
