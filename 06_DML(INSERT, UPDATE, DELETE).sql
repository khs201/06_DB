...
SELECT * FROM EMPLOYEE2 e WHERE EMP_ID = '900'; -- 장채현 확인
...
UPDATE EMPLOYEE2 
SET (SALARY, BONUS) = (
												SELECT SALARY, BONUS FROM EMPLOYEE2 WHERE EMP_NAME = '유재식' -- 다중열 서브쿼리
)
...
JOIN DEPARTMENT2 ON (DEPT_ID = DEPT_CODE)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE LOCAL_NAME LIKE 'ASIA%';  -- 18행 수정 확인
